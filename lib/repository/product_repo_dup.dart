...
//   final Map<String, String> _brandNameCache = {}; // Add this cache
//   Future<List<ProductModel>> _mapProducts(
//       List<QueryDocumentSnapshot> docs) async {
//     if (docs.isEmpty) return [];

//     try {
//       // 1. Batch fetch all variants at once
...
//       // 2. Create a map of product ID to variants
//       final variantsByProduct = <String, List<QueryDocumentSnapshot>>{};
//       for (var variantDoc in variantsSnapshot.docs) {
//         final variantData = variantDoc.data() as Map<String, dynamic>;
//         final productId = variantData['productId'] as String;
//         variantsByProduct[productId] ??= [];
//         variantsByProduct[productId]!.add(variantDoc);
//       }

//       // 3. Batch fetch all size stocks
//       final variantIds = variantsSnapshot.docs.map((doc) => doc.id).toList();
//       final sizeStocksSnapshot = await _firestore
//           .collection('sizes_and_stocks')
//           .where('variantId', whereIn: variantIds)
//           .get();

//       // 4. Create a map of variant ID to size stocks
//       final sizeStocksByVariant = <String, List<SizeStockModel>>{};
//       for (var stockDoc in sizeStocksSnapshot.docs) {
//         final stockData = stockDoc.data() as Map<String, dynamic>;
//         final variantId = stockData['variantId'] as String;
//         sizeStocksByVariant[variantId] ??= [];
//         sizeStocksByVariant[variantId]!
//             .add(SizeStockModel.fromMap(stockData, stockDoc.id));
//       }

//       // 5. Process each product with its pre-fetched data
...
//       return products;
//     } catch (e) {
//       print('Error in _mapProducts: $e');
//       rethrow;
//     }
//   }

//   // Add this new method for cached brand fetching
...
// // Add a method to fetch all brands
...
//           .map((doc) => {
//                 'id': doc.id,
//                 'name': doc.data()['brandName'] as String,
//               })
//           .toList();
//     } catch (e) {
//       rethrow;
